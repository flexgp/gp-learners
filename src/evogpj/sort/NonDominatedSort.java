 ...
 * @author Dylan Sherry
 */
public class NonDominatedSort extends Operator {
	 ...
	 */
	//TODO dylan make these methods lazy
	public static void sort(Population p,
			LinkedHashMap<String, FitnessFunction> f) throws DominationException {
		...
		// now sort population by domination count
		//p.sortByDominationCount();
	}
	...
	public static boolean domination(Individual a, Individual b, LinkedHashMap<String, FitnessFunction> fitnessFunctions )
			throws DominationException {
		Set<String> aFuncNames = a.getFitnessNames();
		Set<String> bFuncNames = b.getFitnessNames();
		if (aFuncNames.size() != bFuncNames.size())
			throw new DominationException(("Error: individuals' fitnesses are not of same "
					+ "length: a=\"%s\" and b=\"%s\"").format(a.toString(),
					b.toString()));
