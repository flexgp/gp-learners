...
import evogpj.evaluation.cuda.SRCorrelationCuda;
import evogpj.evaluation.cuda.SRCorrelationCudaDual;
import evogpj.evaluation.cuda.SRCudaDual;
    ...
    protected void create_operators(Properties props, long seed) throws IOException {
        ...
        for (String fitnessOperatorName : fitnessFunctions.keySet()) {
            if (fitnessOperatorName.equals(Parameters.Operators.SR_JAVA_FITNESS)) {
                ...
                fitnessFunctions.put(fitnessOperatorName, esrf);
            } else if (fitnessOperatorName.equals(Parameters.Operators.SR_CUDA_FITNESS_DUAL)) {
                // this loads the data into shared memory
                DataCuda ed = new DataCuda(PROBLEM, 1);
                ed.readAndStoreDataset();
                int numberOfFeatures = ed.getNumberOfFeatures();
                ...
                SRCudaDual esrf = new SRCudaDual(FUNC_SET, UNARY_FUNC_SET, PROBLEM,numberOfFitnessCases, numberOfFeatures,TARGET_NUMBER, EXTERNAL_THREADS, MEAN_POW,COERCE_TO_INT,0);
                fitnessFunctions.put(fitnessOperatorName, esrf);
            } else if (fitnessOperatorName.equals(Parameters.Operators.SR_CUDA_FITNESS_CORRELATION)) {
                // this loads the data into shared memory
                DataCuda ed = new DataCuda(PROBLEM, 1);
                ed.readAndStoreDataset();
                int numberOfFeatures = ed.getNumberOfFeatures();
                int numberOfFitnessCases = ed.getNumberOfFitnessCases();
                ed = null;
                modelScalerCuda = new SRModelScalerCuda(FUNC_SET, UNARY_FUNC_SET, PROBLEM, numberOfFitnessCases,TERM_SET.size(), TARGET_NUMBER, EXTERNAL_THREADS, MEAN_POW, COERCE_TO_INT);
                if (TERM_SET == null) {
                    TERM_SET = new ArrayList<String>();
                    for (int i = 0; i < numberOfFeatures; i++){
                        TERM_SET.add("X" + (i + 1));
                ...
                SRCorrelationCuda esrf = new SRCorrelationCuda(FUNC_SET, UNARY_FUNC_SET, PROBLEM,numberOfFitnessCases, numberOfFeatures,TARGET_NUMBER, EXTERNAL_THREADS, MEAN_POW,COERCE_TO_INT,0);
                fitnessFunctions.put(fitnessOperatorName, esrf);
            } else if (fitnessOperatorName.equals(Parameters.Operators.SR_CUDA_FITNESS_CORRELATION_DUAL)) {
                // this loads the data into shared memory
                DataCuda ed = new DataCuda(PROBLEM, 1);
                ed.readAndStoreDataset();
                int numberOfFeatures = ed.getNumberOfFeatures();
                ...
                SRCorrelationCudaDual esrf = new SRCorrelationCudaDual(FUNC_SET, UNARY_FUNC_SET, PROBLEM,numberOfFitnessCases, numberOfFeatures,TARGET_NUMBER, EXTERNAL_THREADS, MEAN_POW,COERCE_TO_INT,0);
                fitnessFunctions.put(fitnessOperatorName, esrf);
